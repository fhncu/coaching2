...
namespace TwoBitMachines.FlareEngine
{
        ...
        public class WorldCollision
        {
                ...
                public enum Edge2DSkip { OnDownHold, OnDownPress }
                ...
                public void Move (ref Vector2 velocity, bool hasJumped, ref bool onSurface)
                {
                        if (transform == null || Time.deltaTime == 0) return;

                        HideLayer ( );

                        IgnoreBlockCollision ( );

                        JumpFromMovingPlatform (ref velocity);
                        ...
                        RotateToSlope (velocity);

                        if (onGround) onSurface = true; // if on ground, onSurface is true
                        if (onSurface) onGround = true; // if on bridge (OnSurface), onGround is true
                        velocity = oldVelocity; //        box.velocity was the total velocity applied to the character

                        LatchToMovingPlatform ( );
                }

                public void MoveBasic (ref Vector2 velocity)
                {
                        if (transform == null || Time.deltaTime == 0) return;
                ...
                private void VerticalCollision (ref Vector2 velocity, bool skip = false)
                {
                        ...
                        for (int i = 0; i < box.rays.y; i++)
                        {
                                ...
                                if (useMovingPlatform)
                                {
                                        ...
                                        {
                                                if (velocity.y > 0) continue; //                jump out from a moving platform if inside it
                                                hit.distance = box.skin.y;
                                        }
                                }
                ...
                public bool IgnoreEdge2D (Transform edge, float sign)
                {
                        ...
                        if (sign < 0 && ((skipDownEdge == Edge2DSkip.OnDownHold && holdingDown) || (skipDownEdge == Edge2DSkip.OnDownPress && pressedDown)))
                        {
                ...
                private bool ValidateSlope (ref Vector2 velocity, float magnitude, float slopeAngle, RaycastHit2D slope, ref bool isJumping)
                {
                        ...
                        Vector2 corner = box.BottomExactCorner (velocity.x);
                        ...
                        for (int i = 0; i < 25; i++) // loop for organic slopes, most checks between 1-3
                        {
                                ...
                                RaycastHit2D hit = Physics2D.Raycast (corner, newVelocity.normalized, magnitude, collisionLayer);
                ...
                #region Rotate
                private void RotateToSlope (Vector2 velocity)
                {
                        if (!climbSlopes || !rotateToSlope) return;
                        ...
                        else if (!onGround && rectifyInAir && velocity.y > 0)
                        {
                                ...
                                RotatePlayer (jumpPoint, rotate);
                        }
                }

                private void RotatePlayer (Vector2 pivot, Vector2 slopeNormal)
                {
                        float angle = Vector2.Angle (box.up, slopeNormal);
                        float maxAngle = Mathf.Clamp (angle * Time.deltaTime * rotateRate * 10f, 0, angle);
                ...
                private void VerticalSpikeCheck (ref Vector2 velocity, bool force = false)
                {
                        if (force || (climbSlopes && velocity.y < 0 && (!onGround || Vector2.Angle (groundNormal, box.up) >= 0.01f)))
                        {
                                ...
                                Vector2 size = new Vector2 (box.size.x, box.sizeY * 0.5f);
                ...
                {
                        if (!mp.ignoreBlockCollision) return;
                ...
                public void JumpFromMovingPlatform (ref Vector2 velocity)
                {
                        if (useMovingPlatform && onGround && velocity.y > 0)
                        {
                                mp.Launch (ref velocity);
                        }
                }
                ...
                public void Reset ( )
                {
                        ...
                        if (transform != null) transform.eulerAngles = Vector3.zero;
                }
