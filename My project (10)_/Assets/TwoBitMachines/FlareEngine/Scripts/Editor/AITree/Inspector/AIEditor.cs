...
namespace TwoBitMachines.FlareEngine.Editors
{
        ...
        public class AIEditor : Editor
        {
                ...
                public static List<string> conditionalList = new List<string> ( );
                ...
                private void OnEnable ( )
                {
                        ...
                        if (dataList.Count == 0)
                        {
                                Util.GetFolderStructure ("TwoBitMachines", "/FlareEngine/Scripts/AI/BehaviorTree/Blackboard", "Blackboard", dataList);
                        }
                        if (actionList.Count == 0)
                        {
                                Util.GetFolderStructure ("TwoBitMachines", "/FlareEngine/Scripts/AI/BehaviorTree/Nodes/Action", "Action", actionList);
                        }
                        if (conditionalList.Count == 0)
                        {
                                Util.GetFolderStructure ("TwoBitMachines", "/FlareEngine/Scripts/AI/BehaviorTree/Nodes/Conditional", "Conditional", conditionalList);
                        }
                ...
                public override void OnInspectorGUI ( )
                {
                        ...
                        parent.Update ( );
                        {
                                ...
                                if (Application.isPlaying && fsm.currentState != null)
                                {
                                        ...
                                        EditorGUILayout.LabelField ("Current State:   " + fsm.currentState.stateName);
                                        EditorGUILayout.LabelField ("Current Child:   " + fsm.root.fsmChild);
                                }
                        }
                ...
                public void ShowStates (SerializedProperty states, Color color, bool isAlways = false)
                {
                        for (int i = 0; i < states.arraySize; i++)
                        {
                                ...
                                FoldOut.Bar (state, color, space : 25)
                                        ...
                                        .RightButton ("signal", "Signal", toolTip: "Default Signal", execute: !isAlways && state.Bool ("foldOut"))
                                        .RF ("type", 16, -4, 3)
                                        .BlockRight (20, color, 21)
                                        .LabelRight (StateType (state.Enum ("type")), xOffset : 22);
                                ...
                                if (Bar.FoldOpen (state.Get ("foldOut")))
                                {
                                        ...
                                        {
                                                FoldOut.BoxSingle (1, Tint.Orange * Tint.WarmWhiteB);
                                                {
                                                        state.FieldAndEnable ("", "defaultSignal", "useSignal", execute: !isAlways);
                                                        Labels.Label ("Default Signal", Layout.GetLastRect (100, 18, 14, 2), Tint.White);
                                                }
                                                Layout.VerticalSpacing (2);
                                        }
                                        DisplayNodes (state, state.Get ("action"), Tint.Orange, !isAlways);
                                }
                ...
                public void ResetState (SerializedProperty state, Color color)
                {
                        ...
                        if (open)
                        {
                                DisplayNodes (state, state.Get ("action"), Tint.Orange, false);
                        }
                        if (state.ReadBool ("add"))
                        {
                ...
                private void DisplayNodes (SerializedProperty state, SerializedProperty nodes, Color color, bool showNextState = true)
                {
                        ...
                        for (int i = 0; i < nodes.arraySize; i++)
                        {
                                ...
                                element.Update ( );
                                {
                                        ...
                                        inspectNode.hideFlags = HideFlags.HideInInspector;
                                        SerializedProperty nodeElement = nodes.Element (i);
                                        bool isOpen = element.Bool ("foldOut");

                                        FoldOut.Bar (element, color * Tint.WarmWhiteB, 25)
                                        ...
                                        {
                                                if (inspectNode == null || !inspectNode.OnInspector (fsm, element, color, onEnable)) // if not custom inspector, display it raw
                                                {
                                                ...
                                                {
                                                        FoldOut.Box (2, Tint.Orange, yOffset: -2);
                                                        {
                ...
                private void NodeContextMenu (Vector2 stateIndex)
                {
                        ...
                        {
                                menu.AddItem (new GUIContent (conditionalList[i]), false, SearchForState, conditionalList[i]);
                        }
                        menu.ShowAsContext ( );
                }
