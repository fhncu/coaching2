...
namespace TwoBitMachines.FlareEngine.Editors
{
        ...
        public class DrawNodes
        {
                ...
                {
                        if (icons == null || icons.Count == 0) EditorTools.LoadGUI ("TwoBitMachines", "/FlareEngine/AssetsFolder/GUI/DrawNodesGUI", icons);
                        SetStyle ( );
                        GetTextures ( );
                        SetColors ( );
                }
                ...
                {
                        if (selfInterruptTexture == null) selfInterruptTexture = icons.GetIcon ("SelfInterrupt");
                        if (lowerInterruptTexture == null) lowerInterruptTexture = icons.GetIcon ("LowerInterrupt");
                        if (compositeTexture == null) compositeTexture = icons.GetIcon ("CompositeFrame");
                        if (decoratorTexture == null) decoratorTexture = icons.GetIcon ("DecoratorFrame");
                        if (guardTexture == null) guardTexture = icons.GetIcon ("Guard");
                        if (selectFrame == null) selectFrame = icons.GetIcon ("SelectFrame");
                        if (nodeTexture == null) nodeTexture = icons.GetIcon ("FrameSquare");;
                        if (trashTexture == null) trashTexture = icons.GetIcon ("d_TreeEditor.Trash");;
                        if (circleEmpty == null) circleEmpty = icons.GetIcon ("CircleEmpty");
                        if (circle == null) circle = icons.GetIcon ("CircleJoin");
                        if (hide == null) hide = icons.GetIcon ("EyeClosed");
                        if (unhide == null) unhide = icons.GetIcon ("EyeOpen");

                        if (failedTexture == null) failedTexture = icons.GetIcon ("X");
                        if (checkTexture == null) checkTexture = icons.GetIcon ("Check");
                        if (recheckTexture == null) recheckTexture = icons.GetIcon ("Recheck");
                }
                ...
                public Rect Draw (AITree tree, Node node, bool debug, Vector2 parentMoved, Vector2 gridOffset, Event currentEvent)
                {
                        ...
                        if (nTexture == null) nTexture = Texture2D.whiteTexture;
                ...
                {
                        if (!childIsActive || node.nodeParent == null) return;
                ...
                {
                        if (!node.CanHaveChildren ( ) || node.Children ( ) == null) return;
                ...
                {
                        if (node is Root) return; // root can't look for a parent
                ...
                {
                        if (node.hideChildren || !node.CanHaveChildren ( ) || node.Children ( ) == null) return;
