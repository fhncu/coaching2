...
namespace TwoBitMachines.FlareEngine.Editors
{
        public class InputInspectorEditor
        {
                ...
                public static void Display (SerializedObject parent, SerializedProperty inputs, ThePlayer.Player main, PlayerEditor pEditor)
                {
                        ...
                        if (FoldOut.Bar (parent, Tint.Blue).Label ("Inputs", Color.white).FoldOut ("inputFoldOut"))
                        {
                                for (int i = 0; i < inputs.arraySize; i++)
                                {
                                        ...
                                        input.Update ( );
                                        {
                                                ...
                                                if (FoldOut.FoldOutButton (input.Get ("foldOut"), yOffset: -3))
                                                {
                ...
                private static InputButtonSO CreateScriptableObject (SerializedProperty inputs, string name, ThePlayer.Player main)
                {
                        ...
                        {
                                if (main.inputs.inputSO[i] == null) continue;
                                if (main.inputs.inputSO[i].buttonName == name) containsName = true;
                        }
                ...
                private static void SetInputsButton (InputButtonSO button, string name, KeyCode key, bool isMouse = false)
                {
                        ...
                        if (isMouse) serObj.Get ("type").enumValueIndex = 1;
                        if (isMouse) serObj.Get ("mouse").enumValueIndex = 0;
                ...
                {
                        Rect dropArea = Layout.CreateRectAndDraw (Layout.longInfoWidth + 4, 27, xOffset: -13, texture : Icon.Get ("RoomExit"), color : Tint.Blue);
                        Labels.Centered (dropArea, "Add Existing Input", Color.white, fontSize : 12, shiftY : 0);
