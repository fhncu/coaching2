...
namespace TwoBitMachines.FlareEngine.Editors
{
        ...
        public class ReactionProfileEditor : UnityEditor.Editor
        {
                ...
                private void OnEnable ( )
                {
                        ...
                        {
                                reactionList = Util.GetFileNames ("TwoBitMachines", "/FlareEngine/Scripts/Reactions/Reactions");
                                reactionListSize = reactionList.Length;
                        }
                        for (int i = main.reaction.Count - 1; i >= 0; i--)
                        {
                                if (main.reaction[i] == null) main.reaction.RemoveAt (i);
                                else main.reaction[i].hideFlags = HideFlags.HideInInspector;
                        }
                }
                ...
                public override void OnInspectorGUI ( )
                {
                        ...
                        parent.Update ( );
                        {
                                ...
                                for (int i = 0; i < reactions.arraySize; i++)
                                {
                                        ...
                                        if (reaction != null) reaction.hideFlags = HideFlags.HideInInspector;
                                        reactionObj.Update ( );
