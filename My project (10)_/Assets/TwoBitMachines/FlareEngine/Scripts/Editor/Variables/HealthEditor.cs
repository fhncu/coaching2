...
namespace TwoBitMachines.FlareEngine.Editors
{
        ...
        public class HealthEditor : UnityEditor.Editor
        {
                ...
                public override void OnInspectorGUI ( )
                {
                        ...
                        parent.Update ( );
                        {
                                if (FoldOut.Bar (parent, Tint.Blue).Label ("Health:   " + parent.String ("variableName")).FoldOut ( ))
                                {
                                        ...
                                        Fields.EventFoldOut (parent.Get ("onMinValue"), parent.Get ("minFoldOut"), "On Min Value", execute : eventOpen);
                                        Fields.EventFoldOut (parent.Get ("onMaxValue"), parent.Get ("maxFoldOut"), "On Max Value", execute : eventOpen);
                                        Fields.EventFoldOut (parent.Get ("onValueChanged"), parent.Get ("changedFoldOut"), "On Value Changed", execute : eventOpen);
                                        Fields.EventFoldOut (parent.Get ("onLoadConditionTrue"), parent.Get ("loadFoldOutTrue"), "On Load Condition True", execute : eventOpen);
                                        Fields.EventFoldOut (parent.Get ("onLoadConditionFalse"), parent.Get ("loadFoldOutFalse"), "On Load Condition False", execute : eventOpen);
                                        Fields.EventFoldOut (parent.Get ("onShield"), parent.Get ("shieldFoldOut"), "On Shield Hit", execute : eventOpen && parent.Bool ("hasShield"), color : FoldOut.boxColorLight);
                ...
                public static void CreateScriptableObject<T> (SerializedObject parent, string name, string folderName) where T : ScriptableObject
                {
                        ...
                        if (wf != null)
                        {
                                if (parent != null) parent.Get ("soReference").objectReferenceValue = wf;
                        ...
                        if (parent != null) parent.Get ("soReference").objectReferenceValue = asset;
                }
        }
}