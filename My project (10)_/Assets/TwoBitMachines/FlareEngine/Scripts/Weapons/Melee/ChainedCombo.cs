...
namespace TwoBitMachines.FlareEngine
{
        ...
        public class ChainedCombo
        {
                ...
                public bool Reset ( )
                {
                        ...
                        goToNextMelee = false;
                        inMelee = false;
                        if (collider2DRef != null) collider2DRef.enabled = false;
                        return true;
                }
                ...
                public bool Attack ( )
                {
                        ...
                        if (mouseOverUI && EventSystem.current.currentSelectedGameObject != null && EventSystem.current.currentSelectedGameObject.gameObject.CompareTag ("UIControl")) mouseOverUI = false;
                        return !mouseOverUI && ValidateInputs ( );
                }

                public bool ValidateInputs ( )
                {
                        if (input2 == null && input != null) return input.Active (trigger);
                        return input != null && input2 != null ? input.Active (trigger) && input2.Active (trigger2) : false;
                }

                public bool ExecuteMelee (AnimationSignals signals, int direction, bool onGround, Vector2 position, float coolDown, ref Vector2 velocity)
                {
                        clearSignal = false;
                        bool isFirst = false;
                        if (firstFrame)
                        {
                                ...
                                for (int i = 0; i < combo.Count; i++) combo[i].Reset ( );
                                targetList.Clear ( );
                        }

                        if (goToNextMelee && SequenceFailed (isFirst || Attack ( ) || earlyFlag, onGround, coolDown)) // play after runmeleeattack so animation signals can clear for a least a frame or they might get stuck
                        {
                                return true;
                        }
                        if (clearSignal)
                ...
                private bool SequenceFailed (bool attack, bool onGround, float coolDown)
                {
                        ...
                        if (attack) //    go to next combo
                        {
                                ...
                                if (!GoToNextAttack (onGround, true))
                                {
                                        return Reset ( );
                                }
                                if (index >= combo.Count && attack && coolDown <= 0) // loop
                                {
                                        if (!GoToNextAttack (onGround, false))
                                        {
                                                return Reset ( );
                                        }
                                }
                ...
                private bool GoToNextAttack (bool onGround, bool increment)
                {
                        index = increment ? index + 1 : 0;
                        for (int i = index; i < combo.Count; i++)
                        {
                                if (i < 0 || combo[i].isLocked || combo[i].Skip (onGround))
                                {
                                        index = i + 1;
                                        continue;
                                }
                                else if (combo[i].Exit (onGround))
                                {
                                        return false;
                                }
                                else
                ...
                {
                        combo[index].PlayAnimation (signals, direction, originalDirection, onGround, position, ref velocity); // set animation signal and apply velocity
        ...
        public class MeleeCombo
        {
                ...
                {
                        onHit.Invoke (ImpactPacket.impact.Set (hitWE, collider.bounds.center + Vector3.right * collider.bounds.extents.x * direction, Vector2.zero));
                }

                public void PlayAnimation (AnimationSignals signals, int direction, int originalDirection, bool onGround, Vector2 position, ref Vector2 playerVelocity)
                {
                        ...
                        {
                                firstFrame = false;
                                onMeleeBegin.Invoke (ImpactPacket.impact.Set (meleeWE, position, Vector2.zero));
                        }
                ...
                public void ApplyVelocity (AnimationSignals signals, bool onGround, ref Vector2 playerVelocity, int direction)
                {
                        ...
                        if (velYType != MeleeVelocityY.NoVelocity && !recoilYApplied)
                        {
                                ...
                                {
                                        playerVelocity.y = velY * direction;
                                }
                        }
                ...
                public bool Exit (bool onGround)
                {
                        if (condition == AttackCondition.Anytime) return false;
                        if (condition == AttackCondition.MustBeOnGroundOrExit && !onGround) return true;
                        if (condition == AttackCondition.MustBeInAirOrExit && onGround) return true;
                        return false;
                }

                public bool Skip (bool onGround)
                {
                        if (condition == AttackCondition.Anytime) return false;
                        if (condition == AttackCondition.MustBeOnGroundOrSkip && !onGround) return true;
                        if (condition == AttackCondition.MustBeInAirOrSkip && onGround) return true;
                        return false;
                }
        }
