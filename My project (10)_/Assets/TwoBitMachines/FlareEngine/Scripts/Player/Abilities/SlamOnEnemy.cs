...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
        ...
        public class SlamOnEnemy : Ability
        {
                ...
                public override void ExecuteAbility (AbilityManager player, ref Vector2 velocity, bool isRunningAsException = false) // since using EarlyExecute, this will ALWAYS execute. No need for priority.
                {
                        ...
                        {
                                begin = false;
                                onSlamBegin.Invoke (ImpactPacket.impact.Set (slamBeginWE, this.transform, player.world.boxCollider, player.world.position, this.transform, player.world.box.up, 0));
                        }
                        ...
                        if (player.world.onGround)
                        {
                                ...
                                {
                                        onSlam.Invoke (ImpactPacket.impact.Set (slamWE, this.transform, player.world.boxCollider, player.world.position, this.transform, player.world.box.up, 0));
                                }
                                if (ground)
                                {
                                        holding = true;
                ...
                public override bool OnInspector (SerializedObject controller, SerializedObject parent, string[] inputList, Color barColor, Color labelColor)
                {
                        ...
                        {
                                FoldOut.Box (6, FoldOut.boxColorLight, extraHeight : 5, yOffset: -2);
