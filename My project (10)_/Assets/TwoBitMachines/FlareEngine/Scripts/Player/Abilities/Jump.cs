...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
        ...
        public class Jump : Ability
        {
                ...
                [System.NonSerialized] private float minJumpVel;
                ...
                public override bool IsAbilityRequired (AbilityManager player, ref Vector2 velocity)
                {
                        ...
                        player.jumpButtonHold = player.inputs.Holding ("Jump");
                        player.jumpButtonPressed = player.inputs.Pressed ("Jump");
                        player.jumpButtonReleased = player.inputs.Released ("Jump");
                ...
                public override void ExecuteAbility (AbilityManager player, ref Vector2 velocity, bool isRunningAsException = false)
                {
                        if ((player.ground || coyote) && (player.jumpButtonActive || buffer))
                        {
                                ...
                                onJump.Invoke (ImpactPacket.impact.Set (jumpWE, transform, player.world.boxCollider, player.world.position, null, player.world.box.down, 0));
                                return;
                        }
                        ...
                        if (checkForAirMomentum && player.inputX == 0)
                        {
                        ...
                        if (checkForAirJump)
                        {
                                if (player.jumpButtonPressed && airJumpCount < airJumps && airJumpCount++ < airJumps) // double check to keep airJump signal integrity
                                {
                                        ...
                                        onAirJump.Invoke (ImpactPacket.impact.Set (airJumpWE, this.transform, player.world.boxCollider, player.world.position, null, player.world.box.down, 0));
                                        airGliding = false;
                                }
                                if (player.ground)
                                {
                                ...
                                {
                                        if (airJumpCount == 1) player.signals.Set ("airJump");
                                        else if (airJumpCount == 2) player.signals.Set ("airJump1");
                                        else if (airJumpCount == 3) player.signals.Set ("airJump2");
                                        else if (airJumpCount == 4) player.signals.Set ("airJump3");
                                }
                        }
                        ...
                        // }
                        if ((velocity.y < 0 || (airGliding && velocity.y < player.maxJumpVel * glideBoost)) && checkForFloat && (airJumpCount >= airJumps || glideImmediately) && player.inputs.Holding (glideButton))
                        {
                ...
                public override bool OnInspector (SerializedObject controller, SerializedObject parent, string[] inputList, Color barColor, Color labelColor)
                {
                        if (Open (parent, "Jump", barColor, labelColor))
                        {
                                ...
                                FoldOut.Box (2, FoldOut.boxColorLight);
                                {
                                        ...
                                        parent.Field ("Momentum", "airMomentum");
