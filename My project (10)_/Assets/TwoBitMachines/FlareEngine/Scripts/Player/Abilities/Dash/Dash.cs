...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
        ...
        public class Dash : Ability
        {
                ...
                public override void ExecuteAbility (AbilityManager player, ref Vector2 velocity, bool isRunningAsException = false)
                {
                        ...
                        if (player.wasJumping && player.world.wasOnGround && !player.ground)
                        {
                                Reset (player);
                                checkAirDash = false;
                                player.dashAirJumpCheck = false;
                ...
                private void DoubleTap (AbilityManager player, Vector2 velocity, int pressed)
                {
                        if (pressed != 0)
                        {
                                if (tapped == 0) pressedOld = pressed;
                                tapped++;
                        }
                ...
                {
                        ImpactPacket impact = ImpactPacket.impact.Set (onStartWE, this.transform, player.world.boxCollider, transform.position, null, -Vector2.right * dash.directionX, 0);
                        onStart.Invoke (impact);
                }

                public void OnEndEvent (AbilityManager player)
                {
                        ImpactPacket impact = ImpactPacket.impact.Set (onEndWE, this.transform, player.world.boxCollider, transform.position, null, -Vector2.right * dash.directionX, 0);
                        onEnd.Invoke (impact);
                }
                ...
                public override bool OnInspector (SerializedObject controller, SerializedObject parent, string[] inputList, Color barColor, Color labelColor)
                {
                        if (Open (parent, "Dash", barColor, labelColor))
                        {
                                FoldOut.Box (5, FoldOut.boxColorLight, extraHeight : 5, yOffset: -2);
                                {
                                        ...
                                        parent.Field ("Button Taps", "taps", execute : taps == 0);
                                        ...
                                        parent.DropDownList (inputList, "Extra Button", "extraDashButton");
                                }
                                ...
                                FoldOut.Box (4, FoldOut.boxColorLight);
                                {
                                        ...
                                        parent.FieldToggle ("On Ground Only", "mustBeOnGround");
                                        parent.FieldToggle ("Nullify Gravity", "nullifyGravity");
                                }
                                Layout.VerticalSpacing (5);
                        }
                        return true;
