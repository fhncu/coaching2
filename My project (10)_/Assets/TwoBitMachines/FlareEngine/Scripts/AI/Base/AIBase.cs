...
namespace TwoBitMachines.FlareEngine.AI
{
        ...
        public class AIBase : Character
        {
                ...
                public void PauseDealingDamage (float time)
                {
                        damage.pauseDamage = true;
                ...
                public void Collision (Vector2 velocity, ref float gravity)
                {
                        if (type == CharacterType.Regular)
                        {
                                if (!root.pauseCollision)
                                {
                                        ...
                                        world.Move (ref velocity, root.hasJumped, ref root.onSurface);
                                        world.box.Update ( );
                                }
                                else
                                {
                        ...
                        else if (type == CharacterType.MovingPlatform)
                        {
                                if (MovingPlatformHasGravity && !root.pauseCollision)
                                {
                                        ...
                                        if (Time.deltaTime != 0)
                                        {
                                                movingPlatform.UpdatePlatform (velocity / Time.deltaTime, ref root.isShaking, false);
                                        }
                                }
                                else
                                {
                ...
                public static void AIType (Transform transform, SerializedObject parent, ref bool createUnits, bool isFSM = false)
                {
                        ...
                        if (type == CharacterType.Regular || parent.Bool ("movingPlatformGravity"))
                        {
                                ...
                                CollisionSettings (parent, parent.Get ("world"), parent.Get ("gravity"));
                        }

                        if (FoldOut.Bar (parent, FoldOut.boxColor).Label ("Settings", FoldOut.titleColor, false).FoldOut ("settingsFoldOut"))
                        {
                                if (isFSM)
                                {
                                        FoldOut.Box (5, FoldOut.boxColor, yOffset: -2);
                                        {
                        ...
                        if (FoldOut.Bar (parent.Get ("damage"), FoldOut.boxColor).Label ("Damage", FoldOut.titleColor, false).BRE ("canDealDamage").FoldOut ("damageFoldOut"))
                        {
                                ...
                                FoldOut.Box (4, FoldOut.boxColor, yOffset: -2);
                                {
                                        ...
                                        damage.Field ("Amount", "damage");
                                        damage.Field ("Force", "force");
                                }
                                Layout.VerticalSpacing (3);
                ...
                public static void CollisionSettings (SerializedObject parent, SerializedProperty world, SerializedProperty gravity)
                {
                        if (FoldOut.Bar (parent, FoldOut.boxColor).Label ("Collision", FoldOut.titleColor, false).FoldOut ("collisionSettings"))
                        {
                                FoldOut.Box (3, FoldOut.boxColor, yOffset: -2);
                                {
                                ...
                                FoldOut.Box (1 + extraHeightS, FoldOut.boxColor);
                                {
                ...
                public static void BlackboardDisplay (SerializedObject parent, SerializedProperty bar, AIBase ai, List<string> dataList)
                {
                        if (bar.intValue <= 1)
                        {
                                return;
                        ...
                        for (int i = 0; i < dataArray.arraySize; i++)
                        {
                                ...
                                element.objectReferenceValue.hideFlags = HideFlags.HideInInspector;
                ...
                public static void CreateUnits (AIBase ai)
                {
                        ...
                        {
                                if (ai.units[i] != null) DestroyImmediate (ai.units[i]);
                        }
                        ai.units.Clear ( );
                        for (int i = 0; i < newUnits.Count; i++)
                        {
                ...
                public static bool SetRef (List<Blackboard> data, SerializedProperty field, int index)
                {
                        ...
                        if (typeName != "Pathfinding" && typeName != "PathfindingBasic" && EditorTools.ValidateType (typePath)) // detect if field is blackboard data type
                        {
                                ...
                                {
                                        if (data[j] == null || data[j] is TreeVariable) continue;
                ...
                private static SerializedProperty GetRefName (SerializedProperty array, int id)
                {
                        ...
                        {
                                if (i == id) return array.Element (i);
                        }
                        array.arraySize++;
