                        ...
                        for (int x = 0; x < map.linesX; x++)
                        {
                                ...
                                for (int y = 0; y < map.linesY; y++)
                                {
                                        ...
                                        if (NodeIsAir (map, nodePosition))
                                        {
                                                ...
                                                {
                                                        grid[y * map.linesX + x] = new PathNode ( ) { position = nodePosition, gridX = x, gridY = y, ground = true };
                                                        AddVisualConnection (map, nodePosition, map.cellSize / 4f, Color.green, 1);
                                                }
                                                else
                                                {
                                                        grid[y * map.linesX + x] = new PathNode ( ) { position = nodePosition, gridX = x, gridY = y, air = true };
