...
namespace TwoBitMachines.Editors
{
        public class ConnectCornersEditor : AIPathfindingEditor
        {
                ...
                private static bool CornersNotRelativeToEachOther (Pathfinding map, PathNode cornerA, PathNode cornerB)
                {
                        ...
                        if (cornerA.leftCorner && cornerB.rightCorner && cornerA.gridX > cornerB.gridX && cornerB.gridY >= cornerA.gridY) return false; // valid left edge
                        if (cornerA.rightCorner && cornerB.leftCorner && cornerA.gridX < cornerB.gridX && cornerB.gridY >= cornerA.gridY) return false;
                        return true;
                }

                private static bool CornersWithinJumpingDistance (Pathfinding map, PathNode cornerA, PathNode cornerB)
                {
                        bool withinDistanceX = Mathf.Abs (cornerA.gridX - cornerB.gridX) <= map.maxJumpDistance;
                        bool withinDistanceY = Mathf.Abs (cornerA.gridY - cornerB.gridY) <= map.maxJumpHeight;
                ...
                {
                        if (cornerA.leftCorner && cornerB.rightCorner && cornerA.ShiftX (map, -1) == null && cornerB.gridX < (cornerA.gridX - 1)) return true; // wall is between corners. Corner might already have a connection on said wall. Exit out.
                        if (cornerA.rightCorner && cornerB.leftCorner && cornerA.ShiftX (map, 1) == null && cornerB.gridX > (cornerA.gridX + 1)) return true;
                        return false;
                }
                ...
                private static bool HorizontalBlock (PathNode[] grid, Pathfinding map, PathNode cornerA, PathNode cornerB, int y, int g = 0)
                {
                        ...
                        {
                                for (int searchX = cornerA.gridX; searchX <= cornerB.gridX; searchX++)
                                {
                                        PathNode connectNode = grid[y * map.linesX + searchX];
                                        if (connectNode != null && (cornerA.Same (connectNode) || cornerB.Same (connectNode))) continue;
                                        if (connectNode == null || connectNode.block || !connectNode.air || connectNode.height < 1)
                                        {
                                                return true;
                                        }
                                        if (cornerA.gridY == cornerB.gridY && connectNode.height <= 1)
                                        {
                                                return true;
                                        }
                                        if (searchX < cornerB.gridX) airNode.Add (connectNode);
                                }
                        }
                        return false;
                }
                ...
                {
                        for (int searchY = cornerA.gridY + 1; searchY <= y; searchY++)
                        {
