...
namespace TwoBitMachines.FlareEngine
{
        ...
        public struct PathfindingJob : IJob
        {
                ...
                public void Execute ( )
                {
                        ...
                        while (openSet.Length > 0)
                        {
                                ...
                                for (int x = -1; x <= 1; x++)
                                        for (int y = -1; y <= 1; y++)
                                        {
                                                ...
                                                if (checkX >= 0 && checkX < linesX && checkY >= 0 && checkY < linesY)
                                                {
                                                        ...
                                                        if ((nextNode.height < characterSizeY && !nextNode.ceiling)) continue; //tile.ground &&
                                ...
                                for (int i = 0; i < neighbors.Length; i++)
                                {
                                        ...
                                        int moveCostToNeighbour = nodeCost[node.index].gCost + GetDistance (node.gridX, node.gridY, neighbour.gridX, neighbour.gridY); // nodeCost[node.index].penalty;
                                        ...
                                        if (moveCostToNeighbour < nodeCost[neighbour.index].gCost || !openSet.Contains (neighbour.index))
                                        {
                                                ...
                                                int hCost = GetDistance (neighbour.gridX, neighbour.gridY, target.gridX, target.gridY);
                        ...
                        {
                                if (start.ground && GetDistance (start.gridX, start.gridY, target.gridX, target.gridY) < nearestDistance) // star path is actually closer, stay here
                ...
                {
                        if (target.air && !target.moving && !target.bridge)
                        {
                                for (int i = 1; i <= 5; i++)
                                        if ((target.gridY - i) >= 0)
                                        {
                                                PathNodeStruct ground = jobGrid[(target.gridY - i) * linesX + target.gridX];
                                                if (ground.ground || ground.jumpThroughGround || ground.moving || ground.bridge)
                                                {
                                                        target = ground;
                                                        return;
                                                }
