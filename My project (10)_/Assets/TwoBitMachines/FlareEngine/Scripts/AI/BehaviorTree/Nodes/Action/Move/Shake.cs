...
namespace TwoBitMachines.FlareEngine.AI
{
        ...
        public class Shake : Action
        {
                ...
                public override NodeState RunNodeLogic (Root root) // Belongs to state
                {
                        ...
                        if (shakeActive && Time.deltaTime != 0)
                        {
                                Vector2 resetVelocity = (startPosition - root.position) / Time.deltaTime;
                                Vector2 shakeVelocity = ShakeNow ( ) / Time.deltaTime;
                ...
                public void ResetShake (Root root)
                {
                        ...
                        if (type == ShakeType.Sine) shakeTarget = new Vector2 (-Random.Range (0.0f, 360f), Random.Range (0.0f, 1000f));
                }
                ...
                public Vector2 ShakeNow ( )
                {
                        ...
                        if (type == ShakeType.Random)
                        {
                                ...
                                shakeVelocity = Vector3.Lerp (shakeVelocity, shakeTarget, Time.deltaTime * speed * 100f);
                        }
                        else if (type == ShakeType.Perlin)
                        {
                                ...
                                if (++easeIn < 5f) shakeVelocity *= (easeIn / 4f); // ease in for 4 frames, prevents hard jump.
                        }
                        else
                        {
                        ...
                        if (percent >= 1) shakeActive = false; // is shake complete?
