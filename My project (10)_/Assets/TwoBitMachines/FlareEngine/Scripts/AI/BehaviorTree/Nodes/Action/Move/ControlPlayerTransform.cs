...
namespace TwoBitMachines.FlareEngine.AI
{
        ...
        public class ControlPlayerTransform : Action
        {
                ...
                {
                        if (player == null || position.Count == 0) return NodeState.Failure;
                ...
                {
                        if (index >= position.Count) return Vector2.zero;
                ...
                private void MovePlayer (Root root)
                {
                        ...
                        player.transform.position = Vector2.MoveTowards (start, destination, rate * Time.deltaTime);
                        player.transform.position = new Vector3 (player.transform.position.x, player.transform.position.y, z);
                        start = player.transform.position;
                        player.abilities.ClearVelocity ( );
                ...
                public override bool OnInspector (AIBase ai, SerializedObject parent, Color color, bool onEnable)
                {
                        ...
                        FoldOut.Box (4 + array.arraySize, color, yOffset: -2);
                        {
                        ...
                        Fields.EventFoldOut (parent.Get ("onComplete"), parent.Get ("foldOutEvent"), "On Complete", color : color);
                        return true;
                }
