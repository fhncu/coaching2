...
namespace TwoBitMachines.FlareEngine.AI
{
        ...
        public class PushBack : Action
        {
                ...
                {
                        if (skip) return;
                        if (spriteRenderer != null && spriteRenderer.material != originMaterial) spriteRenderer.material = originMaterial;
                        if (spriteRenderer != null) spriteRenderer.color = originColor;
                        if (spriteEngine != null) spriteEngine.Pause (false);
                ...
                public override NodeState RunNodeLogic (Root root)
                {
                        ...
                        if (!pushBack) return NodeState.Failure;

                        if (beginPushBack)
                        {
                                if (pushBackCalls <= 1f) characterDirection = root.signals.characterDirection; // pushbackcalls prevents resetting direction if pushback is already active
                                beginPushBack = false;
                                if (spriteEngine != null) spriteEngine.Pause (true);
                        }

                        if (pushCounter == 0 && Time.deltaTime != 0)
                        {
                        ...
                        appliedForce -= acceleration * forceDirection * Time.deltaTime;
