...
namespace TwoBitMachines.FlareEngine.AI
{
        ...
        public class FollowGraphPoints : Action
        {
                ...
                {
                        if (targetPoints == null) return NodeState.Failure;
                        ...
                        if (left != null && left.Pressed ( )) FindTarget (root.position, lookLeft : true);
                        if (right != null && right.Pressed ( )) FindTarget (root.position, lookRight : true);
                        if (down != null && down.Pressed ( )) FindTarget (root.position, lookDown : true);
                        if (up != null && up.Pressed ( )) FindTarget (root.position, lookUp : true);

                        if (isActive && Time.deltaTime != 0)
                        {
                                Vector2 newPosition = Vector2.MoveTowards (root.position, currentTarget, Time.deltaTime * speed);
                                root.velocity = (newPosition - root.position) / Time.deltaTime;
                                if ((root.position - currentTarget).sqrMagnitude == 0) isActive = false;
                        }
                ...
                private void FindTarget (Vector2 position, bool lookRight = false, bool lookLeft = false, bool lookUp = false, bool lookDown = false)
                {
                        ...
                        {
                                if (lookRight && (point[i].x <= position.x || Mathf.Abs (point[i].y - position.y) > 0.25f)) continue;
                                if (lookLeft && (point[i].x >= position.x || Mathf.Abs (point[i].y - position.y) > 0.25f)) continue;
                                if (lookDown && (point[i].y >= position.y || Mathf.Abs (point[i].x - position.x) > 0.25f)) continue;
                                if (lookUp && (point[i].y <= position.y || Mathf.Abs (point[i].x - position.x) > 0.25f)) continue;
