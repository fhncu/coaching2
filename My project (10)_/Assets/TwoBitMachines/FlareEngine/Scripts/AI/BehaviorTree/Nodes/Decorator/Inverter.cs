...
namespace TwoBitMachines.FlareEngine.AI
{
        ...
        public class Inverter : Decorator
        {
                ...
                public override bool InterruptLogic (Root root, bool selfAbort = false)
                {
                        ...
                        if (children.Count == 0) return selfAbort ? true : false; // this is not a true exit, no need to invert
                        ...
                        {
                                if (child.RunNodeLogic (root) != NodeState.Success) return true; // invert this
                        }
                        else if (child.isInterruptType)
                        {
                                if (!(child as Composite).InterruptLogic (root, selfAbort)) return true; // invert this
                        }
                        else if (selfAbort)
                        {
