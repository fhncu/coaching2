...
namespace TwoBitMachines.FlareEngine.AI
{
        ...
        public class Root : Composite
        {
                ...
                {
                        if (state.useSignal) signals.Set (state.defaultSignal);
                ...
                public string RunFSMParallel (List<StateNode> action)
                {
                        for (int i = 0; i < action.Count; i++)
                        {
                                stateNode = action[i];
                                NodeState childState = stateNode.node.RunChild (this);
                                if (childState == NodeState.Success && stateNode.goToSuccess) return stateNode.onSuccess;
                                if (childState == NodeState.Failure && stateNode.goToFailure) return stateNode.onFailure;
                        }
                        return "";
                }
                #endregion
