...
namespace TwoBitMachines.FlareEngine.AI.BlackboardData
{
        ...
        public partial class Pathfinding : Blackboard
        {
                ...
                [SerializeField, HideInInspector] public List<NeighborList> neighbor; // this list should actually exist inside AIPathNode, but the inspector crashed every time this AIAction get's instantiated
                ...
                [SerializeField, HideInInspector] private List<TargetPathfinding> unit = new List<TargetPathfinding> ( );
                ...
                public void RegisterFollower (TargetPathfinding newUnit)
                {
                ...
                private void OnDestroy ( )
                {
                        ...
                        if (jobGrid.IsCreated) jobGrid.Dispose ( );
                ...
                public void InitializeJobSystem ( )
                {
                        ...
                        for (int i = 0; i < neighbor.Count; i++)
                                for (int j = 0; j < neighbor[i].neighbor.Count; j++)
                                {
                                        ...
                                        jobNeighbors.Add (index, (int) neighbor[i].neighbor[j].gridX + (int) neighbor[i].neighbor[j].gridY * linesX);
                                }
                }
                #endregion
                ...
                private void PathExtras (bool edit, List<Vector2Int> list, Color color, float cellSize, bool isFall = false)
                {
                        if (edit && grid.Length > 0 && linesX != 0 && linesY != 0)
                        {
                                ...
                                if (node != null)
                                {
                                        SceneTools.Circle (node.position, cellSize / 1.97f, color, 5f);
                                        if (Event.current.type == EventType.MouseDown && Event.current.button == 0)
                                        {
                                                Vector2Int index = new Vector2Int (node.gridX, node.gridY);
                ...
                private void ShowExtras (List<Vector2Int> list, Color color, float cellSize)
                {
                        for (int i = list.Count - 1; i >= 0; i--)
                        {
                                ...
                                if (index < grid.Length && index >= 0)
                                {
                                        ...
                                        SceneTools.Circle (position, cellSize / 1.8f, color, 5f);
                                }
                                else
                                        list.RemoveAt (i);
                        }
        ...
        public class NeighborList
        {
                ...
                public void DrawNeighbor ( )
                {
                        ...
                        if (WorldManager.viewDebugger)
                        {
                                for (int i = 0; i < neighbor.Count; i++)
                                {
                                        Draw.Circle (neighbor[i].position, 0.55f, Color.blue);
                                }
                        }
                        #endif
                        #endregion
