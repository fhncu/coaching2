...
namespace TwoBitMachines.FlareEngine.AI.BlackboardData
{
        ...
        public class TargetTransform : Blackboard
        {
                ...
                {
                        if (target != null) target.position = vector3 + (Vector3) offset;
                }

                public override void Set (Vector2 vector2)
                {
                        if (target != null) target.position = vector2 + offset;
                }
                ...
                public override void OnSceneGUI (UnityEditor.Editor editor)
                {
                        ...
                        if ((oldPosition.x != newPosition.x || oldPosition.y != newPosition.y) && target != null && !Application.isPlaying) // && TwoBitMachines.Editors.Mouse.ctrl)
                        {
