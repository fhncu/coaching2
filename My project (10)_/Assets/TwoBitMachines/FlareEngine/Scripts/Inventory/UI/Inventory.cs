...
namespace TwoBitMachines.FlareEngine
{
        ...
        public class Inventory : MonoBehaviour
        {
                ...
                {
                        if (inventorySO == null || initialized) return;
                        ...
                        {
                                slot.SetSlotItem (inventorySO.list[i], GetItemIcon (inventorySO.list[i].name), reset : true);
                        }
                ...
                public void OnEnable ( )
                {
                        ...
                        if (pauseType == PauseGameType.BlockPlayerInput) ThePlayer.Player.BlockAllInputs (true);
                        onOpen.Invoke ( );
                        if (slot.count > 0 && slot.gridIndex <= 0) slot.Get (0).OnSelect (null);
                        if (slot.count > 0 && slot.gridIndex <= 0) EventSystem.current.SetSelectedGameObject (slot.Get (0).gameObject);
                }
                public void OnDisable ( )
                {
                        if (pauseType == PauseGameType.PauseGame) WorldManager.get.Unpause ( );
                        if (pauseType == PauseGameType.BlockPlayerInput) ThePlayer.Player.BlockAllInputs (false);
                        onClose.Invoke ( );
                }
                private void LateUpdate ( )
                {
                        ...
                        if (navigation == SlotNavigation.Manual)
                        {
                                if (buttonLeftUp != null && buttonLeftUp.Pressed ( )) MoveLeftUp ( );
                                if (buttonRightDown != null && buttonRightDown.Pressed ( )) MoveRightDown ( );
                        }
                ...
                {
                        if (autoItemLoad && slot.count > 0)
                        {
                                UseItem (slot.Get (0).itemUI, false);
                        }
                }
                ...
                {
                        if (inventorySO != null) MoveSlots (inventorySO.ItemList (slot.viewItems, slot.keyName), -1, 1);
                }
                ...
                private void MoveSlots (List<ItemUI> list, int left, int right)
                {
                        ...
                        slot.Carousel (list, totalItems, this);
                        AutoItemLoad ( );
                }
                #endregion
        }
