...
namespace TwoBitMachines.TwoBitSprite.Editors
{
        public static class SpriteProperty
        {
                ...
                public static Texture2D[] PropertyIcons ( )
                {
                        Texture2D[] array = new Texture2D[propertyTypes.Length];
                        for (int i = 0; i < array.Length; i++)
                        {
                                array[i] = (Texture2D) EditorGUIUtility.IconContent (propertyTypes[i].Method.Name + " Icon").image;
                        }
                        return array;
                }
                ...
                public static List<string> NameList ( )
                {
                        ...
                        for (int i = 0; i < propertyTypes.Length; i++)
                                list.Add (propertyTypes[i].Method.Name);
                        return list;
                }
                ...
                public static void DeleteArrayElement (SerializedProperty array)
                {
                        for (int i = 0; i < array.arraySize; i++)
                        {
                                SerializedProperty element = array.Element (i).Get ("delete");
                                if (element == null) continue;
                                if (element.boolValue)
                                {
                                        ...
                                        EditorGUIUtility.ExitGUI ( ); // this will exit out of the inspector
                                        break;
                                }
                        }
                }
                ...
                public static void Execute (SerializedProperty property, SerializedProperty frameIndex, ref bool propertyOpen)
                {
                        ...
                        {
                                if (property.Element (i) == null) continue;
                                SerializedProperty element = property.Element (i);
                                if (element.Get ("property") == null) continue;

                                Bar.Setup (Texture2D.whiteTexture, Tint.SoftDark, space : false, height : 22);
                                Bar.SpaceLeft (25);
                                Rect propertyRect = new Rect (Bar.barStart) { y = Bar.barStart.y + 2, width = Layout.longInfoWidth - 80, height = 18 };
                                EditorGUI.ObjectField (propertyRect, element.Get ("property"), GUIContent.none);
                                Bar.ButtonRight (element.Get ("view"), "Reopen", Tint.White, Tint.White);
                                Bar.ButtonRight (element.Get ("delete"), "xsMinus", Tint.Delete, Tint.Delete, execute : element.Bool ("view"));
                ...
                public static void SetExtraProperty (SpriteEngine main, bool spriteExists, int frameIndex)
                {
                        if (!spriteExists || Application.isPlaying || main.sprites[main.spriteIndex] == null) return;

                        SpritePacket sprite = main.sprites[main.spriteIndex]; // set property values so you can see the actual changes during editor time
