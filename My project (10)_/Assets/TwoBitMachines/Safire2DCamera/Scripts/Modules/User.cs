...
namespace TwoBitMachines.Safire2DCamera
{
        ...
        public class UserPan
        {
                ...
                {
                        if (!enable || pause) return Vector2.zero;

                        if (useKeyboard)
                        {
                                Vector2 speed = Vector2.one * follow.speed;
                                if (Input.GetKey (left)) velocityTarget.x = -speed.x;
                                if (Input.GetKey (right)) velocityTarget.x = speed.x;
                                if (Input.GetKey (down)) velocityTarget.y = -speed.y;
                                if (Input.GetKey (up)) velocityTarget.y = speed.y;
                        }
                ...
                private Vector2 EdgePan (Follow follow, Zoom zoom, Vector3 cameraPosition, Vector3 mousePosition)
                {
                        ...
                        if (mousePosition.y > topBorder && topEdge > 0) velocity.y = boost * follow.speed;
                        if (mousePosition.x < leftBorder && leftEdge > 0) velocity.x = -boost * follow.speed;
                        if (mousePosition.x > rightBorder && rightEdge > 0) velocity.x = boost * follow.speed;
                        if (mousePosition.y < bottomBorder && bottomEdge > 0) velocity.y = -boost * follow.speed;
                        return velocity;
                }
