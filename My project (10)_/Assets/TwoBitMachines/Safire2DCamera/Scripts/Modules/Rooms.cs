...
namespace TwoBitMachines.Safire2DCamera
{
        ...
        public class Rooms
        {
                ...
                public void Reset ( )
                {
                        if (roomExists) rooms[index].onExit.Invoke ( );
                        ignoreClamps = false;
                        transition = false;
                        inTwoRooms = false;
                        index = -1;
                ...
                {
                        if (!enable || rooms.Count == 0) return false;

                        if (TransitionActive (previousCamera)) return true;
                        ...
                        if (!inTwoRooms && roomsFound > 0 && newRoomIndex != index && newRoomIndex != -1)
                        {
                                if (roomExists) rooms[index].onExit.Invoke ( );
                                if (roomExists) inTwoRooms = rooms[index].bounds.Overlap (rooms[newRoomIndex].bounds);
                                previousIndex = index;
                                index = newRoomIndex;
                        ...
                        if (roomsFound == 0 && roomExists && rooms[index].canExit)
                        {
                                zoom.lockZoom = false;
                                if (rooms[index].canZoom || rooms[index].multipleTargets.enable) zoom.Set (1f, 1f);
                ...
                public void ImmediateEnter (Camera camera, Vector2 characterPosition)
                {
                        ...
                        for (int i = 0; i < rooms.Count; i++)
                        {
                                ...
                                {
                                        index = i;
                                        rooms[index].onEnter.Invoke ( );
                                        rooms[index].onTransitionComplete.Invoke ( );
                                        rooms[index].SetTransitionPoint (characterPosition, camera.transform, follow, zoom);
                                        if (rooms[index].canZoom) zoom.Set (rooms[index].ZoomValue (zoom), 0.001f);
                                        camera.transform.position = rooms[index].transitionPoint;
                                        return;
                                }
                        }
                }
                ...
                {
                        if (!enable || transition || ignoreClamps || rooms.Count == 0 || !roomExists) return;
                        rooms[index].Clamp (camera, previousCamera);
                }

                public Vector2 MultipleTarget (Vector2 target, bool isUser)
                {
                        if (!enable || transition || !roomExists || !rooms[index].multipleTargets.enable || isUser) return target;
                ...
                {
                        if (!enable || !roomExists) return target;
                        ...
                        if (Follow.Open (parent, "Rooms", barColor, labelColor, true, canView : true))
                        {
                                ...
                                for (int i = 0; i < array.arraySize; i++)
                                {
                                        ...
                                        FoldOut.BoxSingle (1, Tint.Blue, 0);
                                        {
                                                ...
                                                if (Fields.ConstructButton ("Delete")) { array.DeleteArrayElement (i); break; }
                                        ...
                                        FoldOut.Box (7, Tint.Blue, extraHeight : 5, yOffset: -2);
                                        {
                                                Fields.ConstructField ( );
                                                Fields.ConstructString ("Boundary", Layout.labelWidth - 4, 4);
                                                if (Fields.ConstructButton (element.Bool ("clampLeft") ? "LeftSolid" : "LeftBroken", Layout.quint)) { element.Toggle ("clampLeft"); }
                                                if (Fields.ConstructButton (element.Bool ("clampRight") ? "RightSolid" : "RightBroken", Layout.quint)) { element.Toggle ("clampRight"); }
                                                if (Fields.ConstructButton (element.Bool ("clampTop") ? "TopSolid" : "TopBroken", Layout.quint)) { element.Toggle ("clampTop"); }
                                                if (Fields.ConstructButton (element.Bool ("clampBottom") ? "BottomSolid" : "BottomBroken", Layout.quint)) { element.Toggle ("clampBottom"); }
                                                if (element.Bool ("canExit") && Fields.ConstructButton ("RoomExit", Layout.quint)) { element.Toggle ("canExit"); }
                                                if (!element.Bool ("canExit") && Fields.ConstructButton ("RoomClosed", Layout.quint)) { element.Toggle ("canExit"); }
                ...
                {
                        if (!main.rooms.view || !main.rooms.enable) return;
                        ...
                        for (int i = 0; i < main.rooms.rooms.Count; i++)
                        {
                                ...
                                DrawRoom (room, ref bounds.position, ref bounds.size, room.select == i ? Tint.PastelGreen : Tint.Blue, 0.5f);
                                ...
                                {
                                        if (Camera.main != null) bounds.size = Compute.CameraSize (Camera.main);; //Camera.main.CameraSize ( );
                                }
                ...
                public static void DrawRoom (Room room, ref Vector2 position, ref Vector2 size, Color color, float handleSize = 0.25f)
                {
                        SceneTools.DrawAndModifyBounds (ref position, ref size, color, handleSize, false);
                        ...
                        Handles.color = color;
                        if (room.clampLeft) Handles.DrawLine (position, tL);
                        else Handles.DrawDottedLine (position, tL, 16f);

                        if (room.clampTop) Handles.DrawLine (tL, tR);
                        else Handles.DrawDottedLine (tL, tR, 16f);

                        if (room.clampRight) Handles.DrawLine (tR, bR);
                        else Handles.DrawDottedLine (tR, bR, 16f);

                        if (room.clampBottom) Handles.DrawLine (position, bR);
                        else Handles.DrawDottedLine (position, bR, 16f);
        ...
        public class Room
        {
                ...
                public bool SetTransitionPoint (Vector2 targetEntryPoint, Transform camera, Follow follow, Zoom zoom, bool setZoom = true)
                {
                        ...
                        if (lazyClamp)
                        {
                                if (isClampedY) follow.ForceTargetSmooth (false, true);
                                if (isClampedX) follow.ForceTargetSmooth (true, false);
                                if (setZoom) SetZoom (zoom, delay == 0);
                                return false;
                        }
                        ...
                        transitionX = transitionPoint.x != targetEntryPoint.x;
                        transitionY = transitionPoint.y != targetEntryPoint.y;

                        if (!transitionY && isClampedY) follow.ForceTargetSmooth (false, true);
                        if (!transitionX && isClampedX) follow.ForceTargetSmooth (true, false);
                        if (setZoom) SetZoom (zoom, delay == 0);
                ...
                public bool Transition (Camera camera, Vector3 previousCamera, Zoom zoom, Follow follow)
                {
                        ...
                        float ease = EasingFunction.Ease (tween, transitionCounter, transitionTime);
                        ...
                        if (transitionCounter >= transitionTime) onTransitionComplete.Invoke ( );
                        if (transitionCounter >= transitionTime) follow.ForceTargetSmooth ( );
                ...
                public void Clamp (Camera camera, Vector3 previousCamera, bool clampX = true, bool clampY = true)
                {
                        ...
                        if (clampX)
                        {
                                ...
                                if (canClampLeft && (camCenter.x - camSizeX) < bounds.left) camCenter.x = bounds.left + camSizeX;
                                if (canClampRight && (camCenter.x + camSizeX) > bounds.right) camCenter.x = bounds.right - camSizeX;
                                if (canClampLeft && canClampRight && camSizeX > bounds.width) camCenter.x = bounds.center.x;
                        }
                        ...
                        if (clampY)
                        {
                                ...
                                if (canClampTop && (camCenter.y + camSizeY) > bounds.top) camCenter.y = bounds.top - camSizeY;
                                if (canClampBottom && (camCenter.y - camSizeY) < bounds.bottom) camCenter.y = bounds.bottom + camSizeY;
                                if (canClampTop && canClampBottom && camSizeY > bounds.height) camCenter.y = bounds.center.y;
                        }
                ...
                public void RestrictTarget (Transform target)
                {
                        ...
                        if (!insideRestrictArea) return;

                        if (canClampTop && p.y > (bounds.top - 1f)) target.position = Tooly.SetPosition (p.x, bounds.top - 1f, p);
                        if (canClampLeft && p.x < (bounds.left + 1f)) target.position = Tooly.SetPosition (bounds.left + 1f, p.y, p);
                        if (canClampRight && p.x > (bounds.right - 1f)) target.position = Tooly.SetPosition (bounds.right - 1f, p.y, p);
                        if (canClampBottom && p.y < (bounds.bottom + 1f)) target.position = Tooly.SetPosition (p.x, bounds.bottom + 1f, p);
                }
