...
namespace TwoBitMachines.Safire2DCamera
{
        ...
        public class FollowBlocks
        {
                ...
                {
                        for (int i = 0; i < block.Count; i++) block[i].bounds.Initialize ( );
                }
                ...
                {
                        if (!enable || isUser) return target;
                        ...
                        for (int i = 0; i < block.Count; i++)
                                if (i == index || block[i].bounds.Contains (target))
                                {
                                        if (i != index && index >= 0)
                                        {
                                                if (block[index].vertical) follow.ForceTargetSmooth (y: false);
                                                if (block[index].horizontal) follow.ForceTargetSmooth (x: false);
                                        }
                                        if (i != index) entryPoint = target;
                                        index = block[i].exit ? -1 : i;
                                        if (block[i].exit) follow.ForceTargetSmooth ( );
                                        if (index != -1) newTarget = block[i].Execute (target, entryPoint);
                                }
                        return newTarget;
                }
                ...
                {
                        if (!main.follow.followBlocks.view || !main.follow.followBlocks.view) return;
