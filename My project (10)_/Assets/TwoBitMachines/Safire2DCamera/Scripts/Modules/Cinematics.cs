...
namespace TwoBitMachines.Safire2DCamera
{
        ...
        public class Cinematics
        {
                        ...
                        if (Follow.Open (parent, "Cinematics", barColor, labelColor, true, canView : true))
                        {
                                ...
                                for (int i = 0; i < array.arraySize; i++)
                                {
                                        ...
                                        FoldOut.BoxSingle (1, Tint.Orange, 0);
                                        {
                                                ...
                                                {
                                                        targets.arraySize++;
                                                        if (targets.arraySize == 1) targets.LastElement ( ).Get ("position").vector2Value = SceneTools.SceneCenter (Vector2.zero);
                                        ...
                                        if (!cine.Bool ("open")) continue;

                                        FoldOut.Box (3, Tint.Orange, extraHeight : 5, yOffset: -2);
                                        {
                                        ...
                                        for (int j = 0; j < targets.arraySize; j++)
                                        {
                                                ...
                                                {
                                                        if (target.FieldAndButton ("", "position", "Delete")) { targets.DeleteArrayElement (j); break; }
                ...
                {
                        if (!main.cinematics.view || !main.cinematics.enable) return;
        ...
        public class Cinematic
        {
                ...
                private void NextTarget (Vector3 cameraPosition, Follow follow)
                {
                        ...
                        if (index >= targets.Count) onComplete.Invoke ( );
                        if (index >= targets.Count) state = CineState.Hold; //     
                        if (index >= targets.Count - 1) bottomClamp.y = currentPosition.y - Height (follow);
                        if (index == targets.Count - 1 && letterBox != null) letterBox.onEvent.Invoke ( );
                        if (index < targets.Count && targets[index].zoom) follow.zoom.Set (targets[index].zoomScale, targets[index].duration); //    zoom if target is available
                }

                private void Follow (CinematicTarget target, Vector3 cameraPosition, Follow follow)
                {
                        tweenTimer += Time.deltaTime;
                        float ease = EasingFunction.Ease (target.tween, tweenTimer / target.time);
                        ...
                        {
                                if (target.followX == CineFollow.Ignore) currentPosition.x = cameraPosition.x;
                                if (target.followX == CineFollow.Instant) currentPosition.x = Position (target).x;
                        }
                        if (target.followY != CineFollow.Normal)
                        {
                                if (target.followY == CineFollow.Ignore) currentPosition.y = cameraPosition.y;
                                if (target.followY == CineFollow.Instant) currentPosition.y = Position (target).y + offsetY.y;
                        }
