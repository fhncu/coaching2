...
namespace TwoBitMachines.Safire2DCamera
{
        ...
        public class MultipleTargets
        {
                ...
                public Vector3 FollowMultipleTargets (Vector3 target, bool ignoreClamps)
                {
                        ...
                        if (zoomToFit && !ignoreClamps) ZoomToFit (size);
                        return closest + size;
                }
                ...
                {
                        if (target.x > furthest.x - detect) furthest.x = target.x + detect;
                        if (target.y > furthest.y - detect) furthest.y = target.y + detect;
                        if (target.x < closest.x + detect) closest.x = target.x - detect;
                        if (target.y < closest.y + detect) closest.y = target.y - detect;
                }
                        ...
                        if (FoldOut.Bar (parent, color).Label ("Multiple Targets", FoldOut.titleColor, false).BRE ( ).BR (execute: parent.Bool ("enable")).FoldOut ( ))
                        {
                                ...
                                for (int i = 0; i < targets.arraySize; i++)
                                {
                                        ...
                                        FoldOut.Box (1, color, yOffset: -2);
                                        {
                                                ...
                                                if (Fields.ConstructButton ("Delete")) { targets.DeleteArrayElement (i); break; }
                                        }
                                        Layout.VerticalSpacing (3);
                                }
                                GUI.enabled = true;
