...
namespace TwoBitMachines.Safire2DCamera
{
        ...
        public class Shakes
        {
                ...
                public void Reset ( )
                {
                        ...
                        if (enable) TurnOffAllShakes (immediate: true);
                }
                ...
                {
                        if (!enable || Time.timeScale == 0) return;
                ...
                {
                        for (int i = 0; i < shakes.Count; i++) shakes[i].active = false;
                ...
                public static void CustomInspector (SerializedProperty parent, Color barColor, Color labelColor, Shakes shakeRef, Safire2DCamera main)
                {
                        ...
                        if (Follow.Open (parent, "Shakes", barColor, labelColor, true))
                        {
                                ...
                                if (shake == null || shake.objectReferenceValue == null) return;
                                                ...
                                                FoldOut.BoxSingle (1, Tint.Orange, 0);
                                                {
                                                        ...
                                                        if (Fields.ConstructButton ("Play", Layout.quarter))
                                                        {
                                                                ...
                                                                if (main.cameraRef != null)
                                                                {
                                                                        ...
                                                                        {
                                                                                if (shakeRef.shakes.Count > 0) shakeRef.StopTestMode ( );
                                                        ...
                                                        if (Fields.ConstructButton ("Red", Layout.quarter) && shakeRef != null) { shakeRef.StopTestMode ( ); }
                                                        if (Fields.ConstructButton ("Delete", Layout.quarter)) { shakes.DeleteArrayElement (i); break; }
                ...
                public void TestMode ( )
                {
                        ...
                        RestoreCameraValues ( );
                        Execute ( );
                        if ((AllShakesCompleteTestMode (shakes) && singleShake.trauma <= 0)) StopTestMode ( );
                }
                ...
                public void StopTestMode ( )
                {
                        ...
                        if (camera != null) RestoreCameraValues ( );
                ...
                public void ShakeTestMode (List<ShakeInfo> shakeList, string name)
                {
                        for (int i = 0; i < shakeList.Count; i++)
                                if (name == shakeList[i].name)
                                {
                                        ...
                                        if (shakes != null) Set (s.shakeType, s.speed, s.strength, s.duration, s.constant, TimeScale.TimeUnscaled, s.amplitude);
                                        return;
                                }
                }
                ...
                {
                        if (constantShake.active) return false;
                        for (int i = shakeThis.Count - 1; i >= 0; i--)
                                if (shakeThis[i].active) return false;
                        return true;
                }
        ...
        public class Shake
        {
                ...
                public virtual void Set (ShakeType shakeType, float speed, float strength, float duration, bool constant, TimeScale timeScale, Vector3 amplitude)
                {
                        ...
                        if (shakeType == ShakeType.OneShot) this.amplitude = ((Vector2) amplitude).normalized * -1f;
                        if (shakeType == ShakeType.Sine) target = new Vector3 (0, -Random.Range (0, 360f), Random.Range (0, 1000f));
                }
                ...
                public Vector3 ShakeNow (AnimationCurve damper, AnimationCurve recoil)
                {
                        ...
                        #if UNITY_EDITOR
                        if (!EditorApplication.isPlaying) deltaTime = Clock.unscaledDeltaTime;
                        #endif
                        ...
                        else if (shakeType == ShakeType.Perlin)
                        {
                                ...
                                if (++easeIn < 5f) position *= (easeIn / 4f); // ease in for 4 frames, prevents hard jump.
                        }
