...
namespace TwoBitMachines.Safire2DCamera
{
        ...
        public class WorldBounds
        {
                ...
                {
                        if (!enable || bounds.Count == 0) return;
                        ...
                        if (Follow.Open (parent, "World Bounds", barColor, labelColor, true, canView : true))
                        {
                                ...
                                for (int i = 0; i < array.arraySize; i++)
                                {
                                        ...
                                        FoldOut.BoxSingle (1, FoldOut.boxColor);
                                        {
                                                ...
                                                {
                                                        parent.Get ("select").intValue = ListReorder.destination;
                                                }
                ...
                {
                        if (!main.worldBounds.view || !main.worldBounds.enable) return;
                        ...
                        for (int i = 0; i < bounds.Count; i++)
                        {
                                ...
                                {
                                        if (aChanged) bound.b.x = bound.a.x;
                                        if (bChanged) bound.a.x = bound.b.x;
                                }
                                else
                                {
                                        if (aChanged) bound.b.y = bound.a.y;
                                        if (bChanged) bound.a.y = bound.b.y;
                                }
        ...
        public class WorldBound
        {
                ...
                public Vector3 Clamp (Vector3 center, float previousX, float width, float height)
                {
                        clamping = false;
                        if (boundsType == BoundsType.Left)
                        {
                                float vel = center.x - previousX;
                                if (vel < 0 && (center.x - width) < a.x + buffer)
                                {
