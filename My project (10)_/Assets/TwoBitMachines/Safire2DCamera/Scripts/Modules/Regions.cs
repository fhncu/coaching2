...
namespace TwoBitMachines.Safire2DCamera
{
        ...
        public class Regions
        {
                ...
                public void Reset ( )
                {
                        for (int i = 0; i < regions.Count; i++) regions[i].inside = false;
                }

                public Vector3 Offset (Vector3 target, Follow follow, bool isUser)
                {
                        if (!enable || regions.Count == 0 || isUser) return Vector3.zero;
                ...
                public static void CustomInspector (SerializedProperty parent, Color barColor, Color labelColor)
                {
                        ...
                        if (Follow.Open (parent, "Regions", barColor, labelColor, true, canView : true))
                        {
                                ...
                                for (int i = 0; i < array.arraySize; i++)
                                {
                                        ...
                                        FoldOut.Box (type == 1 ? 4 : 3, color : FoldOut.boxColor, extraHeight : 3, yOffset: -2);
                                        {
                                        ...
                                        bool eventOpen = FoldOut.FoldOutButton (element.Get ("eventsFoldOut"), yOffset: -2);
                ...
                {
                        if (!main.follow.regions.view || !main.follow.regions.enable) return;
                        ...
                        for (int i = 0; i < main.follow.regions.regions.Count; i++)
                        {
                                ...
                                if (isTransform) element.position = element.transform.position;
                                if (!isTransform && element.transform != null) element.transform.position = element.position;
