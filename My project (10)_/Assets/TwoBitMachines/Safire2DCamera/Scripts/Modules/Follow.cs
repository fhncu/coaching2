...
namespace TwoBitMachines.Safire2DCamera
{
        ...
        public class Follow
        {
                ...
                public void Execute ( )
                {
                        ...
                        target = detectWalls.Position (target, screenZone, camera);
                        ...
                        cameraVelocity += screenZone.Velocity (target, camera, isUser);
                        cameraVelocity += peek.Velocity (this, isUser);
                        if (autoScroll.x != 0) cameraVelocity.x = autoScroll.x * Time.deltaTime;
                        if (autoScroll.y != 0) cameraVelocity.y = autoScroll.y * Time.deltaTime;
                ...
                {
                        if (x) { forceSmoothX = true; forceClampX = false; }
                        if (y) { forceSmoothY = true; forceClampY = false; }
                }

                public void ForceTargetClamp (bool x = true, bool y = true)
                {
                        if (x) { forceClampX = true; forceSmoothX = false; }
                        if (y) { forceClampY = true; forceSmoothY = false; }
                }
