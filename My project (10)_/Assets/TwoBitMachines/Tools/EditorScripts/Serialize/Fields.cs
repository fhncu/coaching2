  ...
  namespace TwoBitMachines.Editors
  {
          ...
          public static class Fields
          {
                  ...
                  public static Property Start (SerializedObject property, string label = "", float shorten = 0)
                  {
                          ...
                          Property.C ( );
                          if (label != "") prop.L (label, Layout.labelWidth, shorten);
                          return prop;
                  }
                  ...
                  public static void EventField (SerializedProperty onEvent, float adjustX = 0, Color color = default (Color), bool execute = true)
                  {
                          if (!execute) return;
                          float height = EditorGUI.GetPropertyHeight (onEvent);
                          Rect rect = Layout.CreateRectAndDraw (Layout.longInfoWidth, height, xOffset: -11, color : color, texture : Texture2D.whiteTexture); //, texture : Skin.longBorderFrame, color : Tint.Grey100);
                          Rect rectField = new Rect (rect) { x = rect.x + adjustX * 0.5f, y = rect.y, width = rect.width - adjustX };
                          EditorGUI.PropertyField (rectField, onEvent);
                  }

                  public static void EventFoldOut (SerializedProperty onEvent, SerializedProperty toggle, string label, float adjustX = 0, float yOffset = 0, Color color = default (Color), bool execute = true)
                  {
                          if (!execute) return;
                          ...
                          if (toggle.boolValue)
                          {
                                  ...
                                  float height = EditorGUI.GetPropertyHeight (onEvent);
                                  Rect rect = Layout.CreateRect (Layout.longInfoWidth, height, xOffset: -11); //, texture : Skin.longBorderFrame, color : Tint.Grey100);
                                  Rect rectField = new Rect (rect) { x = rect.x + adjustX * 0.5f, y = rect.y + yOffset, width = rect.width - adjustX };
                          ...
                          {
                                  Rect rect = Layout.CreateRect (Layout.longInfoWidth, 20, xOffset: -11);
                                  Rect rectField = new Rect (rect) { x = rect.x + adjustX * 0.5f, y = rect.y + yOffset, width = rect.width - adjustX, height = 20 };
                                  Skin.Draw (rectField, color : color == default (Color) ? FoldOut.boxColor : color, FoldOut.background);
                                  Labels.Display (rectField.OffsetX (2), label + " ( )", xOffset : 5);
                                  if (Event.current.isMouse && rect.ContainsMouseDown ( ))
                                  {
                                          toggle.boolValue = !toggle.boolValue;
                                  }
                  ...
                  public static void EventFoldOut (SerializedProperty onEvent, string label, float adjustX = 0, float yOffset = 0, Color color = default (Color), bool execute = true)
                  {
                          if (!execute) return;
                  ...
                  public static void EventFoldOutEffect (SerializedProperty onEvent, SerializedProperty effectName, SerializedProperty toggle, string label, Color color = default (Color), bool execute = true)
                  {
                          ...
                          else
                          {
                                  ...
                                  if (effectName != null)
                                  {
                                          ...
                                          if (effectName.stringValue == "") Labels.FieldText ("World Effect", rightSpacing : 3);
                                  }
                          }
                  }
                  ...
                  public static void EventFoldOutEffectAndRate (SerializedProperty onEvent, SerializedProperty effectName, SerializedProperty effectRate, SerializedProperty toggle, string label, Color color = default (Color), bool execute = true)
                  {
                          ...
                          else
                          {
                                  ...
                                  if (effectName != null)
                                  {
                                          ...
                                          Labels.FieldDoubleText (effectName.stringValue == "" ? "Effect" : "", "Rate", rightSpacing : 2f);
                                  }
                          }
                  }

                  public static bool EventFoldOutFloat (SerializedProperty onEvent, SerializedProperty valueFloat, SerializedProperty toggle, string label, Color color = default (Color), int xOffset = 0, bool ySpace = true, bool execute = true)
                  {
                          if (!execute) return false;
                          if (ySpace) Layout.VerticalSpacing (1);
                          bool pressed = false;
                          if (toggle.boolValue)
                          {
                                  float height = EditorGUI.GetPropertyHeight (onEvent);
                  ...
                  public static void ConstructField (int yOffset = 0)
                  {
                          fieldRect = Layout.CreateRectField (yOffset: yOffset);
                          fieldRectOffset = 0;
                  }
                  ...
                  public static void ShowSprite (Sprite sprite, int size = 10, int xOffset = 0, int yOffset = 0, bool execute = true)
                  {
                          if (!execute || sprite == null) return;

                          Rect spriteRect = sprite.rect;
                          Texture2D tex = sprite.texture;
                          Rect rect = fieldRect.Adjust (xOffset, size); //new Rect (fieldRect) { x = fieldRect.x + fieldRectOffset, y = fieldRect.y + yOffset + 3, width = size, height = size };
                          rect.height = size;
                          rect.x += xOffset;
                          rect.y += yOffset;
                  ...
                  public static void ArrayBox (this SerializedProperty array, string title, Color color, int yOffset = 0)
                  {
                          if (array.arraySize == 0)
                          {
                                  array.arraySize++;
                          }
                          FoldOut.Box (array.arraySize, color, yOffset : yOffset);
                          {
                                  for (int i = 0; i < array.arraySize; i++)
                                  {
                          ...
                          Layout.VerticalSpacing (5 + yOffset);
                  }
                  ...
                  public static void FieldClampToZero (this SerializedProperty property, string title, string field)
                  {
                          ...
                          EditorGUI.PropertyField (rect.Adjust (Layout.labelWidth, Layout.contentWidth), property.Get (field), GUIContent.none);
                          if (property.Float (field) < 0) property.Get (field).floatValue = 0;
                  }
                          ...
                          switch (evt.type)
                          {
                                  ...
                                  case EventType.DragPerform:
                                          ...
                                          if (evt.type == EventType.DragPerform)
                                          {
                                                  ...
                                                  foreach (System.Object item in DragAndDrop.objectReferences)
                                                  {
                                                          if (item is T) list.Add (item as T);
                                                  }
                                                  if (list.Count > 0) evt.Use ( );
                                          }
                                          break;
                          }
                  }
                  ...
                  public static void FieldAndEnable (this SerializedProperty property, string title, string field, string toggle, bool execute = true, bool titleIsField = false)
                  {
                          ...
                          if (titleIsField) EditorGUI.PropertyField (rect.Adjust (0, Layout.labelWidth), property.Get (title), GUIContent.none);
                          if (!titleIsField) Labels.Label (title, rect.Adjust (0, Layout.labelWidth));
                  ...
                  public static void FieldAndToggle (this SerializedProperty property, string title, string field, string toggle, bool execute = true)
                  {
                          if (!execute) return;
                          Rect rect = Layout.CreateRectField ( );
                          Labels.Label (title, rect.Adjust (0, Layout.labelWidth));
                  ...
                  {
                          if (!execute || names.Length == 0) return;
                          Rect rect = Layout.CreateRectField ( );
                  ...
                  public static bool InputAndButtonBox (string title, string icon, Color boxColor, ref string name, int buttonWidth = 15, int yOffset = 0)
                  {
                          Rect rect = Layout.Box (boxColor, yOffset : yOffset);
                          rect.x += 7;
                          EditorGUI.LabelField (rect.Adjust (0, Layout.labelWidth), title, editorStyle);
                  ...
                  public static void FieldDouble (this SerializedProperty property, string title, string field1, string field2, bool execute = true, bool titleIsField = false)
                  {
                          if (!execute) return;
                          Rect rect = Layout.CreateRectField ( );
                          if (titleIsField)
